...
export function shortestDistance(p1: Point, p2: Point, p: Point): number {
  ...
  const norm =
    Math.sqrt(lineVec[0] ** 2 + lineVec[1] ** 2 + lineVec[2] ** 2) || 1e-5;
  let distance =
    Math.sqrt(cross[0] ** 2 + cross[1] ** 2 + cross[2] ** 2) / norm;
  ...
  const dot =
    vec[0] * pointVec[0] + vec[1] * pointVec[1] + vec[2] * pointVec[2];
...
export function segmentedLeastSquaresFixedSegments(
  points: Point[],
  numSegments: number
): [number, number[]] {
  ...
  const dp: number[][] = Array.from({ length: n }, () =>
    Array(numSegments + 1).fill(Infinity)
  );
  const result: number[][] = Array.from({ length: n }, () =>
    Array(numSegments + 1).fill(0)
  );
...
export function processPoses(
  ...
  const memIndices: number[][] = keypoints.map(() =>
